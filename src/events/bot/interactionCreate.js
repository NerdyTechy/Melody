        ...
        if (interaction.isChatInputCommand()) {
            ...
            if (interaction.customId.includes("-")) {
                ...
                const button = client.buttons.get(
                    interaction.customId.substring(0, dashIndex)
                );
                if (!button) return;
                try {
            ...
            if (interaction.user.id != buttonOwner) {
                embed.setDescription(
                    `Only <@${buttonOwner}> can use this menu.`
                );
